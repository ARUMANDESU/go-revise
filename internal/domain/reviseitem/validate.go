...
func validateName(name string) error {
	...
	err := validation.Validate(
		...
		validation.Length(1, maxNameLength).
			Error(fmt.Sprintf("name must be between 1 and %d characters", maxNameLength)),
	)
	if err != nil {
		return errs.
...
func validateDescription(description string) error {
	...
	err := validation.Validate(
		...
		validation.Length(1, maxDescriptionLength).
			Error(fmt.Sprintf("description must be between 1 and %d characters", maxDescriptionLength)),
	)
	if err != nil {
		return errs.
...
func validateNextRevisionAt(nextRevisionAt time.Time) error {
	...
	if nextRevisionAt.IsZero() {
		return errs.
			NewUnknownError(op, errs.ErrInvalidInput, "nextRevisionAt is required").
			WithContext("nextRevisionAt", nextRevisionAt)
	}
	if nextRevisionAt.Before(time.Now()) {
		return errs.
			NewUnknownError(op, errs.ErrInvalidInput, "nextRevisionAt is in the past").
			WithContext("nextRevisionAt", nextRevisionAt)
	}
